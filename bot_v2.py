...
def extraer_precios_texto(texto):
    """Extrae precios de texto plano con formato de tabla."""
    resultados = {}
    for linea in texto.split("\n"):
        # Formato con pipes: | 70 | 180 | 357 |
        ...
        if match:
            ...
            continue
        # Formato sin pipes: "  70    180    357  🔴"
        match2 = re.search(r'^\s*(\d{1,3})\s+(\d+)\s+(\d+)', linea)
        if match2:
            num_partido = int(match2.group(1))
            precio = int(match2.group(3))
            if num_partido in ALERTAS:
                resultados[num_partido] = precio
    return resultados

def extraer_precios_embed(embed):
    """Extrae precios de un embed de Discord."""
    resultados = {}
    # Busca en description y fields del embed
    textos = []
    if embed.get("description"):
        textos.append(embed["description"])
    for field in embed.get("fields", []):
        textos.append(field.get("value", ""))
        textos.append(field.get("name", ""))
    
    for texto in textos:
        for linea in texto.split("\n"):
            match = re.search(r'(\d{1,3})\s+(\d+)\s+(\d+)', linea)
            if match:
                num_partido = int(match.group(1))
    ...
    try:
        async with session.get(url, headers=headers) as resp:
            ...
            for mensaje in mensajes:
                ...
                # Intenta leer embeds primero
                for embed in mensaje.get("embeds", []):
                    precios.update(extraer_precios_embed(embed))

                # Si no hay embeds, intenta texto plano
                if not precios:
                    texto = mensaje.get("content", "")
                    if texto:
                        precios.update(extraer_precios_texto(texto))
